...
mkValidator p d () ctx = (traceIfFalse "beneficiary2's signature missing" signedByBeneficiary2 &&
                         traceIfFalse "deadline not reached" deadlineReached) || (traceIfFalse "beneficiary1's signature missing" signedByBeneficiary1 &&
  ...
  where
    ...
    signedByBeneficiary1 = txSignedBy info $ unPaymentPubKeyHash $ p

    signedByBeneficiary2 :: Bool
    signedByBeneficiary2 = txSignedBy info $ unPaymentPubKeyHash $ p

    deadlineReached :: Bool
    deadlineReached = contains (from $ d) $ txInfoValidRange info

    deadlineNotReached :: Bool
    deadlineNotReached = contains (to $ d) $ txInfoValidRange info
