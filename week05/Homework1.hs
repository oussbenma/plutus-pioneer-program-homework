...
mkPolicy pkh deadline () ctx =  traceIfFalse "deadline passed" (to deadline `contains` txInfoValidRange (scriptContextTxInfo ctx))    &&
                                traceIfFalse "wrong public key hash" (txSignedBy (scriptContextTxInfo ctx) $ unPaymentPubKeyHash pkh)
